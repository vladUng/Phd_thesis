...
There are two methods reviewed in this section: \acrlong{wgcna} (WGCNA - \citep{Langfelder2008-sn}) and Parsimonious Gene Correlation Network Analysis (PGCNA - \citep{Care2019-ij}). The former was developed with the goal of combining multiple datasets to stratify a disease, whereas the latter was primarily focused on analysing co-expressed networks as well as the first method to formalise the co-expressed network approach. Despite their distinct goals, the two approaches share common steps in the network construction process but have different caveats:
...
The authors compare their work with WGCNA and draw inspiration from other techniques that re-construct the regulatory network of tissues from microarrays and use Mutual Information (MI - \citep{Margolin2006-mc,Zhang2013-fs}) to calculate gene pair-wise relationship. Compared to correlation scores MI is a more general method to describe the dependence between two variables by using information theory concepts, but it is also harder to interpret.
