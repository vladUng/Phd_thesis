...
There are two methods reviewed in this section Weighted Gene Correlation Network Analysis (WGCNA) \citet{Langfelder2008-sn} and Parsimonious Gene Correlation Analysis (PGCNA) \citet{Care2019-ij}. The former represents the first formalisation of this kind of methods to analysis correlation networks while the latter is a more recent development and targeted to disease subtyping. 

Both methods and the others derived are similar in methodology having the following steps:
\begin{enumerate}
...
To these steps PGCNA also extracts the relevant genes for each communities and uses them to stratify the cancer. In addition, the work from \citet{Care2019-ij} also supports multiple datasets.

The review from 2023 of \citet{Petti2023-qo} is covering the network based approaches to stratify a disease. Another interesting review is the one from \citet{Van_Dam2018-id} which covers the network built from Deferentially Expressed Genes (DEG). Another review from 2020 which focuses on network fusion is \citet{Di_Nanni2020-qu} which also covers the work from \citet{Dimitrakopoulos2018-br}.
