\subsection{Overview} \label{s:lit:co_net}
...
The field of using networks to analyse genomic data is a fast moving field with rapid advancements in the past 10 years. 

The review from 2023 of \citet{Petti2023-qo} is covering the network based approaches to stratify a disease. In this section, the most relevant models to the project presented there are covered. Another interesting review is the one from \citet{Van_Dam2018-id} which covers the network built from Deferentially Expressed Genes (DEG). Another review from 2020 which focuses on network fusion is \citet{Di_Nanni2020-qu} which also covers the work from \citet{Dimitrakopoulos2018}, presented later in \ref{} section,
...
\subsubsection{HCNM} \label{s:lit:HCNM}
...
\subsubsection{iHNMMO} \label{s:lit:iHNMMO}
...
% NetICS
\subsubsection{NetICS} \label{s:lit:netICS}
...
\subsubsection{DrDimont} \label{s:lit:drDimont}
