...
\section{Analysing omics data} \label{s:multi-omics}
...
The previous work done in the field can be categorised based on the data given to the models. This can vary from focusing on how mutations affect the genes pathways, the effect of transcriptomics or processing the signalling pathways. This represents the foundation for multi-modal or in bioinformatics terminology, multi-omics approaches where the hypothesis is different types of data yields better predictions. This project focuses on combining multiple information streams into a single multi-omics approach. The hypothesis is that mutations (covered in \cref{s:mutations}) affect gene expression (\cref{s:rnaSeq}) and considering both will yield a better subtyping of the \acrlong{mibc}.
...
\paragraph*{Autoencoders} \label{s:autoencoders}
