...
Clustering algorithms have many variations and these are best covered in a code sample\cite{Scikit-learn_undated-ax} from the Scikit-learn library\cite{Pedregosa2011-ts}. It is beyond the scope of this report to cover all the techniques but will present the most popular and the ones used in \nameref{s:multi-omics}, which can be seen in Figure \ref{fig:clustering_types}. The rows represent different types of datasets and the columns are the algorithms covered in this document: K-means being a general-purpose algorithm, Ward and Agglomerative both being hierarchical clustering, but Agglomerative is peforming pair-wise grouping. It is worth mentioning, that a modified version of the Scikit-learn code was adapted to the project needs, enabling us to run multiples clustering techniques with varied parameters to a dataset. 
...
In addition to the above metrics, there is a popular heuristic procedure, the Elbow method, which aids in choosing the right number of clusters. In Figure \ref{fig:elbow_method} the y-axis is represented by the distance of the points from their centroids, while the x-axis is the number of clusters. It can be seen that there is a point of inflexion when the number of clusters is set to five and that is considered to be the optimal number of clusters.
...
\begin{figure}[!htb]
  \centering\includegraphics[width=0.5\textwidth,height=0.5\textheight,keepaspectratio]{Images/Resources/basic_graphs.png}
...
% Despite being inspired by biological Neural Networks, the conventional ANN have many differences which may be the cause

\acrfull{snn} are considered the third generation of Artificial NN as they embed additional biological characteristics. The main difference between the conventional ANN and SNN is that in the latter, the neurons fire when receiving enough input in a specific time window; this temporal dimension can be another method to encode information. On top of that, there is research that shows that dendrites can encode information at no or little computational cost \cite{Cao2015-wj}. This has the potential to process information with smaller ANN\footnote{Remember, one of the disadvantages of the DL is that it requires large Artificial Neural Networks that are expensive to train.} and/or to tackle more complex problems (with many features) at a low computational cost. Moreover, there is recent work by \citet{Perez-Nieves2021-uj} which has shown to be promising in building higher level concepts\cite{Stringer2008-mw,Stringer2006-fj} by adding heterogenous synapse time firing. It is worth remembering that the current scepticism in the conventional ANN approaches is that they can't form higher-level representations of objects, so these might be an exciting opportunity to overcome that. On top of that, SNN has been predicted to be more computational efficient and dedicated hardware are built like TrueNorth\cite{Dharmendra_S_Modha_undated-is} (IBM) and  Spinaker\cite{Furber2014-ot} (The University of Manchester). 

However, SNNs are still in their infancy and apart from helping neuroscientists to simulate different parts of the brain, they haven't been applied to more real-life problems. One of the reasons for this is that they don't have a back-propagation equivalent algorithm, but there are current efforts in this area \cite{Nicola2017-ru}. Moreover, the temporal dimension introduces additional complexity into the models making it difficult to research.
