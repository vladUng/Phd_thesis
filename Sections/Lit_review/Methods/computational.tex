...
\begin{lstlisting}[float=!h, caption={Agglomerative hierarchical clustering pseudocode}, label={code:agg_clustering}]
...
One of the challenges in clustering is to measure the performance of grouping as there is no labelling or prior information on how the grouping should look like. This project uses the canonical metrics which are supported by Scikit-learn\cite{Pedregosa2011-ts,Scikit-learn_undated-ax}: Silhouette Coefficient, Calinski-Harabasz and Davies-Bouldin. 

All three scores are used across all the results chapter, in the clustering analysis (\cref{s:cs:right_config,fig:cs:sill_distrib}), first network approach developed in \cref{s:p0:clustering_analysis} and the the final network pipeline in \cref{s:ap:N_II:clustering analysis}.
