...
Another class of methods for community detection algorithms are based on the random walks. The central idea is to analyse the information flow dynamics of a network by randomly going through the graph. There are several methods employing this approach such as InfoMap  \citet{Rosvall2008-kw} or Markov CLustering (MCL) \citet{Van_Dongen2008-yj}. This class is not explored in this project as it suffers from the same limitations of modularity based methods\cite{Peixoto2021-jx}, and is prone to finding communities further from the ground truth in comparison to SBM.
