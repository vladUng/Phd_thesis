...
The previous two chapters presented the first iteration of the network pipeline developed to stratify the \acrfull{mibc} using multiple data types. Data integration occurred at three different stages: 1) non-tumour data at the network level, 2) mutation burden at the edge weights, and 3) prioritisation of \acrfull{tf} at the degree level. All three integrative methods were successfully used and validated through different graph metrics, but from a biological stance, the most successful strategy resulted from modifying the number of edges as covered in the selective edge pruning \cref{s:N_I:sel_pruning}.
...
The work present in this chapter results from applying the improved version of the network pipeline to stratify the MIBC. It includes building a graph from non-tumour RNAseq data, integrating TFs via selective edge pruning and mutations via the reward weight modifier, and both the expression from tumour and non-tumour via IMEI. Throughout the chapter, the standard and reward networks were tested and validated computationally and through biology's lens. 
...
% No IFNG 
The previous Basal split based on the \acrfull{ifn} is not found when using the non-tumour graph representation. This is explained by the fact that the samples in the non-tumour dataset do not show an \acrshort{ifn} response as it is specific to MIBC tumours treated with Bacillus Calmette–Guérin (BCG) that induce inflammatory response \citep{Baker2022-bj}. However, when a more detailed clustering analysis is performed with the reward network, it was found that one of the immune infiltrated subgroups have tendancy of a basal subgroup.


% Main finding network approach works
This chapter's main findings are that data integration into a non-tumour network works and that the network pipeline has the potential to highlight new biology through the analysis performed.
